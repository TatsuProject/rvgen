...
**Current phase:** Phase 1 steps 1–7 complete + **step-8 trap/interrupt/privileged subsystem landed** + Phase 2 crypto + **RVV 1.0 baseline** + **5 Zve* embedded targets (incl. Coral NPU)** + **production-grade functional-coverage infrastructure** + **SV-faithful scalar load/store stream family** + **PyPI-ready pyproject** + **user-area YAML targets (declarative `riscv_core_setting` + per-target CLINT map)**. **396 unit tests passing.** End-to-end CLI pipeline (gen → gcc_compile → iss_sim → cov) passes **51/51** non-vector Spike runs + **18/18** rv64gcv vector runs + **48/48 new privileged/interrupt/exception tests** + **21/21 trace-level matches on chipforge-mcu RTL**. Instruction registry = **486 ops**, stream registry = **16 distinct classes**, **27 built-in targets** + **user-area YAML targets**, **32 covergroups** (18 static + 10 runtime + 4 crosses).

**Latest session (2026-04-24):** user-area + per-target CLINT. User explicitly asked for a clear framework / user-configuration split and pointed out `rvgen/targets/__init__.py` was a poor name for a 552-line module. Landed:
- `rvgen/targets/__init__.py` split into `core_setting.py` (TargetCfg dataclass, mirrors riscv-dv's SV filename), `presets.py` (MMODE_CSRS / USM_EXCEPTIONS / etc. tuples exposed under a `PRESETS` lookup for YAML reference), `builtin.py` (27 framework targets), `loader.py` (YAML loader + user-area discovery).
- `user/` tree at repo root — drop-in zone for user-authored targets (YAML), testlists, directed streams, coverage goals. Resolved via `--user_dir` → `$RVGEN_USER_DIR` → `./user`.
- `user/targets/chipforge-mcu.yaml` — worked example with custom CLINT base (0x10010000) + `unsupported_instr` denylist for SHA-512 split-pair.
- `TargetCfg` extended with `clint_base / msip_offset / mtimecmp_offset / mtime_offset` (SiFive-CLINT defaults) so timer-IRQ emit targets the actual SoC. Also `isa_string / mabi` so YAML targets declare their own toolchain strings.
- `interrupts.py` reads CLINT addrs from `cfg.target`. Side fix: RV32 now writes both halves of MTIMECMP via the spec-correct "high-safe → low-commit → high-commit" sequence (was only writing low half, leaving a latent ~4.3B-cycle cap).
- CLI: `--target_config <yaml>` for one-off YAMLs, `--user_dir <path>`, `--help_targets` listing, `--target` dropped `choices=` restriction so user-area names work.
- `docs/user-area.md` explains the framework/user split.

**Session 2026-04-23 (evening) — trap/interrupt/privileged:** step 8's easier half. User asked for "interrupt handler support, exceptions, privileged mode" explicitly, so paging + PMP + debug ROM were deliberately deferred. Changes:
...
> Read `CLAUDE.md` §0 in `/home/qamar/chipforge/rvgen/` first. 396 unit tests pass, 51/51 Spike E2E baseline + 18/18 rv64gcv vector + 48/48 privileged/interrupt/exception tests + 21/21 chipforge-mcu trace-level match. Trap / exception / interrupt / U-mode-boot subsystem landed (`rvgen/privileged/{trap,boot,interrupts}.py` + `docs/examples/privileged-testlist.yaml`). Target config restructured (`rvgen/targets/{core_setting,presets,builtin,loader}.py`) with user-area YAML targets at `user/targets/*.yaml` — per-target CLINT map + ISA/ABI live on `TargetCfg`. PyPI metadata ready for v0.1.0 upload (pyproject.toml, docs/releasing.md). 32 covergroups, layered CGF goals, directed auto-regression, parallel regression runner in `scripts/regression.py`, CLI tools (merge/diff/attribute/export/report/per-test/baseline-check/suggest-seeds/lint-goals). Docs: `docs/verification-guide.md`, `docs/user-area.md`. Pick the next item from §0 "Next-up queue" and work on it. Keep running `python -m pytest tests/` after every change. Update §0 when a major milestone lands.
